...
bool SignMessage(const ECDSA<ECP, SHA1>::PrivateKey& key, const string& message, string& signature)
{
    ...
    StringSource(message, true,
                 new SignerFilter(prng, ECDSA<ECP, SHA1>::Signer(key),
...
bool VerifyMessage(const ECDSA<ECP, SHA1>::PublicKey& key, const string& message,
...
int main(int argc, char* argv[])
{
    ...
    PrintDomainParameters(publicKey.GetGroupParameters());
